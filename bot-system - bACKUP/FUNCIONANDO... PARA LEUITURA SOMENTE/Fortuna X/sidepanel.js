...
async function executarValidacoesSeguranca(user) {
  try {
    ...
    const { data: userData, error: userStatusError } = await supabase
      .from('usuarios')
      .select('active, installation_id, active_client_id, active_updated_at')
      .eq('id', user.id);
...
chrome.storage.local.get(['rouletteState', 'historicoRodadas'], (result) => {
  if (result.rouletteState) {
    ...
    if (result.rouletteState.configQuentesFrios) {
        if (inputQtdQuentes) inputQtdQuentes.value = result.rouletteState.configQuentesFrios.qtdQuentes;
        if (inputQtdFrios) inputQtdFrios.value = result.rouletteState.configQuentesFrios.qtdFrios;
        if (inputQtdVizinhos) inputQtdVizinhos.value = result.rouletteState.configQuentesFrios.qtdVizinhos;
    }
...
chrome.storage.onChanged.addListener((changes, areaName) => {
  if (areaName === 'local' && changes.historicoRodadas) {
    const novoHist = changes.historicoRodadas.newValue || [];
    console.log('📊 [SIDEPANEL] Histórico atualizado no storage:', novoHist.length);
    state.historicoRodadas = novoHist;
    atualizarListaHistorico();
...
function atualizarListaHistorico() {
  ...
  window.historicoRodadasGlobal = historico;
  ...
  const slider = document.getElementById('sliderHistorico');
  const maxRodadas = slider ? parseInt(slider.value) : 500;
  const rodadasExibidas = historico.slice(0, maxRodadas);
  ...
  const totalSpins = rodadasExibidas.length;

  rodadasExibidas.forEach(val => {
    const num = parseInt(val);
    if (isNaN(num)) return;

    // Para Dúzias, Colunas, Cores e Par/Ímpar, Altos/Baixos
    // Geralmente analistas preferem ver a probabilidade real (incluindo o zero no denominador)
    // para saber a frequência real de acerto.

    // Dúzias
    if (num >= 1 && num <= 12) counts.d1++;
    else if (num >= 13 && num <= 24) counts.d2++;
    else if (num >= 25 && num <= 36) counts.d3++;
    ...
    // Altos/Baixos
    if (num >= 1 && num <= 18) counts.baixo++;
    else if (num >= 19 && num <= 36) counts.alto++;
  ...
  // Usar o total de rodadas (incluindo zeros) como denominador para refletir a frequência real
  const calcPerc = (count) => totalSpins > 0 ? ((count / totalSpins) * 100).toFixed(1) + '%' : '0.0%';

  const updateStat = (eltId, count, total, tipo) => {
    ...
    // Lógica de cores para destacar tendências
    let color = '#fff'; // Padrão
    
    if (tipo === 'duzia' || tipo === 'coluna') {
        if (perc > 37) color = '#00ff00'; // Muito Quente
        else if (perc > 34) color = '#aaffaa'; // Quente
        else if (perc < 28) color = '#ff4444'; // Muito Frio
        else if (perc < 31) color = '#ffaaaa'; // Frio
    } else if (tipo === 'cor' || tipo === 'paridade' || tipo === 'altura') {
        if (perc > 54) color = '#00ff00';
        else if (perc < 44) color = '#ff4444';
    }
    
    el.style.color = color;
    if (el.parentElement) {
      el.parentElement.style.borderColor = color === '#fff' ? '#444' : color;
    }
  };

  updateStat('statD1', counts.d1, totalSpins, 'duzia');
  updateStat('statD2', counts.d2, totalSpins, 'duzia');
  updateStat('statD3', counts.d3, totalSpins, 'duzia');
  updateStat('statC1', counts.c1, totalSpins, 'coluna');
  updateStat('statC2', counts.c2, totalSpins, 'coluna');
  updateStat('statC3', counts.c3, totalSpins, 'coluna');
  updateStat('statPreto', counts.black, totalSpins, 'cor');
  updateStat('statVermelho', counts.red, totalSpins, 'cor');
  updateStat('statAlto', counts.alto, totalSpins, 'altura');
  updateStat('statBaixo', counts.baixo, totalSpins, 'altura');
  updateStat('statPar', counts.par, totalSpins, 'paridade');
  updateStat('statImpar', counts.impar, totalSpins, 'paridade');
}
...
if (btnEstrategias) {
  btnEstrategias.addEventListener('click', () => {
    if (modalEstrategias) modalEstrategias.style.display = 'flex';
  });
}
    ...
    if (selectedOption && selectedOption.dataset) {
      try {
        ...
        const gatilhos = gatilhosRaw.map((g, index) => {
          ...
          let isGale = g.tipo === 'GALE';
          let isCiclo = g.tipo === 'CICLO';
          ...
          if (!g.tipo && g.repeticoes && g.repeticoes > 1) {
            isGale = true;
          }
          ...
          return {
            ...
            gales: (isGale || isCiclo) ? (g.multiplicadores || (g.repeticoes ? Array(g.repeticoes).fill(1) : undefined)) : undefined,
            ciclos: isCiclo ? (g.multiplicadores || undefined) : undefined,
            multiplicadores: g.multiplicadores || undefined,
...
if (btnSimulacao) {
  btnSimulacao.addEventListener('click', () => {
    chrome.storage.local.get(['modoSimulacaoAtivo', 'saldoSimulacao', 'valorFichaSimulacao'], (result) => {
      ...
      if (saldoSimulacaoInput) saldoSimulacaoInput.value = result.saldoSimulacao !== undefined ? result.saldoSimulacao : 1000.00;
      if (fichaSimulacaoInput) fichaSimulacaoInput.value = result.valorFichaSimulacao !== undefined ? result.valorFichaSimulacao : 1.00;
...
if (toggleSimulacaoBot) {
  toggleSimulacaoBot.addEventListener('change', (e) => {
    atualizarTextoStatusSim(e.target.checked);
  });
}
...
if (btnSalvarSimulacao) {
  btnSalvarSimulacao.addEventListener('click', () => {
    ...
    const saldo = parseFloat(saldoSimulacaoInput.value) || 1000.00;
    const ficha = parseFloat(fichaSimulacaoInput.value) || 1.00;
...
function saveState() {
  // Criar uma cópia do estado sem o histórico de rodadas para evitar sobrescrever o background
  const stateParaSalvar = { ...state };
  delete stateParaSalvar.historicoRodadas;
  
  chrome.storage.local.set({ rouletteState: stateParaSalvar });
}
    ...
    historico.forEach(item => {
      ...
      html += `
        <tr style="border-bottom: 1px solid #222;">
          ...
          <td style="padding: 8px; text-align: center;">${item.gale > 0 ? `Gale ${item.gale}` : '-'}</td>
